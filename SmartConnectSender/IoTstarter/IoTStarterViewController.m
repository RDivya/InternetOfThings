...
- (void)viewDidLoad
{
    ...
    temparatureArray = [[NSMutableArray alloc] initWithObjects:@"22", @"25", @"50", @"15", @"60", @"70", @"30", @"1", @"20", @"35", nil];
}
...
- (void)updateAccelLabels
{
    ...
    NSLog(@"self.accelX.text : %@", self.accelX.text);
    NSInteger index = 0;
    if([self.accelX.text integerValue] == 0){
        self.accelX.text = [temparatureArray objectAtIndex:0];
    }else{
        index = [temparatureArray indexOfObject:self.accelX.text];
        index = index+1;
        if([temparatureArray count]-1 >= index){
            self.accelX.text = [temparatureArray objectAtIndex:index];
        }else{
            index = 0;
            self.accelX.text = [temparatureArray objectAtIndex:index];
        }
    ...
    AppDelegate *appDelegate = [[UIApplication sharedApplication] delegate];
    NSString *messageData = [MessageFactory createTextMessage:self.accelX.text];
